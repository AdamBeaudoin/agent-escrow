...
echo "Updated: $JUDGE_ENV"
echo ""
if [[ "$RECIPIENT" == "$ZERO_ADDR" ]]; then
  echo "⚠ Set MPP_RECIPIENT in .env (or re-run: MPP_RECIPIENT=0xYourWallet ./scripts/bootstrap-mainnet.sh)"
fi
echo "Next:"
...
echo "  3) Start judge service: cd \"$JUDGE_DIR\" && npm start"
